...
function initContentScript() {
  ...
  window.addEventListener("fakeFiller:personalFill", doPersonalizedFill);
  ...
  async function doPersonalizedFill() {
    console.log("--- AI-Powered Personalized Fill Initialized ---");

    // Get personalized data and fill mode
    const { personalizedData, personalFillMode } = await chrome.storage.local.get({
      personalizedData: [],
      personalFillMode: 'direct'
    });

    if (!personalizedData || personalizedData.length === 0) {
      showContentToast("Please define your personalized data in the extension popup first.", "error");
      return;
    }

    // Determine if we should use AI
    const useAI = (personalFillMode === 'ai');

    if (useAI) {
      await doPersonalizedFillWithAI(personalizedData);
    } else {
      await doPersonalizedFillDirect(personalizedData);
    }
  }

  /**
   * AI-powered personalized fill that uses AI to intelligently match fields with personal data
   */
  async function doPersonalizedFillWithAI(personalizedData) {
    console.log("--- Using AI Mode for Personalized Fill ---");

    // Show progress overlay
    showProgressOverlay();
    updateProgressOverlay("Initializing AI-Powered Fill", "Analyzing form fields...");

    try {
      // Detect if this is a Google Form
      const isGForm = window.location.hostname === 'docs.google.com' &&
      ...
      updateProgressOverlay(
        "✓ Personalized Fill Complete",
        `Successfully filled ${filledCount} fields with your data.`
      );
      updateProgressBar(1);

      setTimeout(() => {
        hideProgressOverlay();
        showContentToast(`Successfully filled ${filledCount} fields with your data.`, 'success');
      }, 2000);

    } catch (error) {
      console.error("AI Personalized Fill Error:", error);
      updateProgressOverlay("Error", error.message);
      setTimeout(() => {
        hideProgressOverlay();
        showContentToast("An error occurred during AI-powered fill: " + error.message, 'error');
      }, 3000);
    }
  }

  /**
   * Direct keyword-based personalized fill (original method)
   */
  async function doPersonalizedFillDirect(personalizedData) {
    console.log("--- Using Direct Mode for Personalized Fill ---");
    ...
    for (const field of fields) {
      ...
      // Collect all text metadata from the field and its surroundings for more robust matching.
      const metadata = [
        field.name,
        field.id,
        field.placeholder,
        ...
        field.parentElement?.closest('label')?.textContent,
      ].join(' ').toLowerCase();

      if (metadata.trim() === '') continue;

      // Iterate through the user's custom rules.
      for (const rule of personalizedData) {
        const dataValue = rule.value;
        const keywords = rule.keywords.toLowerCase().split(',').map(k => k.trim()).filter(Boolean);

        if (!dataValue || keywords.length === 0) continue;

        let isMatch = false;
        // Strong match by type (if user types 'email' or 'tel' as a keyword)
        if (keywords.includes(fieldType)) {
          isMatch = true;
        }

        // Match by keywords found in metadata
        if (!isMatch && keywords.some(kw => metadata.includes(kw))) {
          isMatch = true;
        }
        ...
        if (isMatch) {
          ...
          field.dispatchEvent(new Event("input", { bubbles: true }));
          field.dispatchEvent(new Event("change", { bubbles: true }));
          break; // Move to the next field once a rule has matched
        }
      }
    }
    ...
    if (filledCount > 0) {
      ...
      showContentToast("Couldn't find any fields to fill with your personal data.", 'info');
    }
  }
  ...
  /**
   * Use AI to determine which personal data best matches a field
   * @param {string} fieldContext - The context/label/description of the field
   * @param {Array} personalizedData - Array of personal data objects
   * @returns {Object|null} - The matched data object or null
   */
  async function getAIMatchForField(fieldContext, personalizedData) {
    try {
      // Create a prompt for AI to match the field with personal data
      const dataList = personalizedData.map((item, index) =>
        `${index + 1}. ${item.key}: "${item.value}" (keywords: ${item.keywords})`
      ).join('\n');

      const prompt = `You are a form-filling assistant. Given a form field and a list of personal data, determine which data best matches the field.
...
Instructions:
- Analyze the field context carefully
- Determine which personal data item is the best match for this field
- Consider the field's purpose, label, and context
- If NO data matches appropriately, respond with "NONE"
- If a match is found, respond ONLY with the number (1, 2, 3, etc.) of the matching data item
...
Response (number only or "NONE"):`;
      ...
      console.log(`AI Match for "${fieldContext}": ${response}`);

      if (response === "NONE" || response === "0") {
        return null;
      }

      // Parse the number from AI response
      const matchNumber = parseInt(response);
      if (isNaN(matchNumber) || matchNumber < 1 || matchNumber > personalizedData.length) {
        console.warn(`Invalid AI response: ${response}`);
        return null;
      }

      return personalizedData[matchNumber - 1];
  ...
  `;
    ...
    document.getElementById('run-ai-button').addEventListener('click', (e) => {
      ...
      if (isProcessing) {
        ...
        toggleProgressOverlay(false); // Show overlay before starting
        doSmartFill();
      }
    }, { capture: true });
